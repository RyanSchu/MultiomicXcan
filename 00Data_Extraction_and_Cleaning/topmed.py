...
for line in gzip.open(chrfile):
    ...
    if(len(ref) > 1 or len(alt) > 1): #do not output indels, PrediXcan only allows SNPs
        continue
    if(re.search('GENOTYPED_ONLY',filter)): #look for 'TYPED' to decide whether to split into 4 or 5
        (altAF, MAF, AC, AN)= info.split(";")
        R2="R2=1"
    elif(re.search('GENOTYPED',filter)): #look for 'TYPED' to decide whether to split into 4 or 5
        (altAF, MAF, R2, ER2, AC, AN)= info.split(";")
    else:
        (altAF, MAF, R2, AC, AN ) = info.split(";")
