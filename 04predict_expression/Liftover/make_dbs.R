...
weights <- read.table('/home/ryan/topmed/multiomic_modeling/dbs/lifted_weights_uniq_CAU.txt', stringsAsFactors =F, header=F)
...
str(model_summaries)
colnames(weights)<-c("gene_id","rsid","varID","ref","alt","beta")
str(weights)
# weights<-weights%>% mutate(rsid="chr" %&% rsid)
        ...
        conn <- dbConnect(drv = driver, '/home/ryan/topmed/multiomic_modeling/dbs/CAU_multiomic_model_hg38.db')
